...
import { AlertTriangle, Clock, Wrench, CheckCircle, Loader2 } from 'lucide-react';
import {
  ...
  return (
    <div className="h-screen flex flex-col bg-[#0d1117] text-white overflow-hidden select-none">
      ...
      >
        ...
        <div className={`text-center px-6 py-2 rounded-xl border ${statusBorder}`}>
          ...
          {intervention && (
            <p className="text-xs text-gray-500 mt-0.5">
              depuis {elapsed(intervention.called_at)}
            </p>
          )}
        </div>
        ...
        <div className="text-right">
          ...
          {state.machine.location && (
            <p className="text-gray-600 text-xs mt-0.5">{state.machine.location}</p>
          )}
        </div>
      </header>
      ...
      <main className="flex-1 flex items-center justify-center px-8 gap-8" style={{ height: '78%' }}>
        ...
        {/* ── Left info panel ─────────────────────────────────────────────── */}
        <div
          className="flex flex-col gap-4 h-full py-6"
        ...
        >
          ...
          <InfoCard
            ...
            value={
              state.last_maintenance_days_ago !== null
                ? `il y a ${state.last_maintenance_days_ago} j`
                : '—'
            }
          />
          <InfoCard
            label="Tickets ouverts"
            value={String(state.open_tickets_count)}
            alert={state.open_tickets_count > 0}
          />
        </div>
        ...
        {/* ── Action zone ─────────────────────────────────────────────────── */}
        <div className="flex-1 flex flex-col items-center justify-center gap-8 max-w-xl">

          {error && (
            <p className="text-red-400 text-sm bg-red-500/10 px-4 py-2 rounded-lg">{error}</p>
          )}

          {/* STATE 1 — idle: big red call button */}
          {isIdle && (
            <>
          ...
          {/* STATE 2 — waiting: amber status + start button */}
          {isWaiting && (
            <>
              <div
              ...
              >
                ...
                <p className="text-amber-600 text-sm mt-1">
                  Appelé il y a {elapsed(intervention!.called_at)}
                </p>
          ...
          {/* STATE 3 — in_progress: blue status + complete button */}
          {isInProgress && (
            <>
              <div
                className="w-full rounded-2xl px-6 py-5 text-center"
              ...
              >
                <Wrench className="mx-auto text-blue-400 mb-2" size={36} />
                <p className="text-blue-300 text-xl font-semibold">Intervention en cours</p>
                <p className="text-blue-600 text-sm mt-1">
                  Démarrée il y a {elapsed(intervention!.started_at!)}
                </p>
                {intervention!.operator_note && (
                  <p className="text-gray-400 text-sm mt-3 italic">"{intervention!.operator_note}"</p>
                )}
              ...
              <button
                disabled={acting}
                onClick={() => act(() => completeIntervention(machine_id!, note || undefined))}
              ...
              >
                {acting ? <Loader2 className="animate-spin mx-auto" size={24} /> : <><CheckCircle className="inline mr-2" size={22} />Terminer l'intervention</>}
              </button>
            </>
          )}
        </div>
          ...
          {state.last_intervention ? (
            <>
              ...
              {state.last_intervention.completed_at && (
                <InfoCard
                  label="Terminée"
                  value={new Date(state.last_intervention.completed_at).toLocaleString('fr-CA', {
