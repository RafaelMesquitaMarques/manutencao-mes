...
const CONFIG_TABS: { id: ConfigTab; label: string; labelKey?: string; Icon: LucideIcon }[] = [
  { id: 'general',    label: 'General',          labelKey: 'equipment.cfgGeneral', Icon: Settings     },
  { id: 'stop',       label: 'Stop Categories',  labelKey: 'nav.stopCategories', Icon: StopCircle   },
  { id: 'reject',     label: 'Reject Categories', labelKey: 'equipment.rejectCategories', Icon: AlertTriangle },
  { id: 'operators',  label: 'Operators',        labelKey: 'equipment.operatorsTitle', Icon: Users        },
  { id: 'shifts',     label: 'Work Shifts',      labelKey: 'equipment.workShifts', Icon: Clock        },
  { id: 'parameters', label: 'Parameters',       labelKey: 'equipment.cfgParameters', Icon: BarChart2    },
  { id: 'indicators',          label: 'Indicators',          labelKey: 'equipment.cfgIndicators', Icon: Activity },
  { id: 'intervention_types', label: 'Intervention Types',  labelKey: 'nav.interventionTypes', Icon: Zap      },
  { id: 'safety_checklist',   label: 'Safety Checklist',    labelKey: 'equipment.safetyChecklist', Icon: Shield   },
...
function StopCategoriesTab({ slug, allMachines, machineId }: { slug: string; allMachines: Machine[]; machineId: string }) {
  ...
  const delCat = async (id: string) => {
    if (!confirm(t('equipment.deleteCatConfirm'))) return;
    try {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-bold text-gray-400 uppercase tracking-widest">{t('nav.stopCategories')}</h2>
        <div className="flex gap-2">
      ...
      {showAdd && (
        <div className="p-4 bg-[#0d1421] rounded-2xl border border-white/[0.06] space-y-3">
          <input value={addForm.name} onChange={(e) => setAddForm((f) => ({ ...f, name: e.target.value }))}
            placeholder={t('equipment.catNamePlaceholder')} className="w-full bg-[#0b1120] border border-white/10 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500" />
          ...
          <div className="flex gap-3 items-center">
            <div><label className="block text-xs text-gray-500 mb-1">{t('common.icon')}</label><IconPicker value={addForm.icon} onChange={(k) => setAddForm((f) => ({ ...f, icon: k }))} /></div>
            <div><label className="block text-xs text-gray-500 mb-1">{t('common.color')}</label>
          ...
          <div className="flex gap-4">
            <Toggle label={t('equipment.commentRequired')} checked={addForm.comment_required} onChange={(v) => setAddForm((f) => ({ ...f, comment_required: v }))} />
            <Toggle label={t('equipment.triggersMaintenance')} checked={addForm.triggers_maintenance} onChange={(v) => setAddForm((f) => ({ ...f, triggers_maintenance: v }))} />
          </div>
          <div className="flex gap-2">
            <button onClick={addCat} disabled={busy || !addForm.name.trim()} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-bold disabled:opacity-50 flex items-center gap-1.5"><Check size={13} /> {t('common.add')}</button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        {cats.length === 0 && <p className="col-span-2 text-center py-10 text-gray-700 text-sm">{t('equipment.noStopCats')}</p>}
      </div>

      {selected && (
        <div className="p-4 bg-[#0b1120] rounded-2xl border border-white/[0.06] space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest">{t('equipment.subcategoriesFor', { name: selected.name })}</h3>
          ...
          {showAddSub && (
            <div className="p-3 bg-[#0d1421] rounded-xl space-y-2">
              ...
              <div className="flex gap-3 items-center">
                ...
                <Toggle label={t('equipment.triggersMaintenance')} checked={addSubForm.triggers_maintenance} onChange={(v) => setAddSubForm((f) => ({ ...f, triggers_maintenance: v }))} />
              </div>
              <div className="flex gap-2">
                <button onClick={addSub} disabled={busy || !addSubForm.name.trim()} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold disabled:opacity-50 flex items-center gap-1"><Check size={11} /> {t('common.add')}</button>
          ...
          <div className="space-y-1.5">
            {(selected.subcategories || []).map((sub: StopSubcategoryOut) => (
              editSubId === sub.id ? (
                ...
                <div key={sub.id} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-white/[0.03]">
                  ...
                  {sub.triggers_maintenance && <span className="text-xs text-orange-400 bg-orange-400/10 px-2 py-0.5 rounded-full">🔧 {t('equipment.maintBadge')}</span>}
            ...
            {(selected.subcategories || []).length === 0 && <p className="text-xs text-gray-700 text-center py-4">{t('equipment.noSubcategories')}</p>}
          </div>
        </div>
      )}
...
function RejectCategoriesTab({ slug, allMachines, machineId }: { slug: string; allMachines: Machine[]; machineId: string }) {
  ...
  const delCat = async (id: string) => {
    if (!confirm(t('equipment.deleteRejectCatConfirm'))) return;
    try {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-bold text-gray-400 uppercase tracking-widest">{t('equipment.rejectCategories')}</h2>
        <div className="flex gap-2">
          <button onClick={() => setShowClone(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-white/[0.05] hover:bg-white/[0.1] text-gray-300 rounded-xl text-xs font-bold border border-white/10 transition-colors">
            <Copy size={12} /> {t('equipment.cloneTo')}
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        {cats.length === 0 && <p className="col-span-2 text-center py-10 text-gray-700 text-sm">{t('equipment.noRejectCats')}</p>}
      </div>
...
const DEFAULT_SHIFTS_DEF = [
  { key: 'morning',   labelKey: 'shift.morning',   defaultStart: '06:00', defaultEnd: '14:00', color: '#f59e0b' },
  { key: 'afternoon', labelKey: 'shift.afternoon', defaultStart: '14:00', defaultEnd: '22:00', color: '#3b82f6' },
  { key: 'night',     labelKey: 'shift.night',     defaultStart: '22:00', defaultEnd: '06:00', color: '#8b5cf6' },
];
...
function WorkShiftsTab({ machineId, savedConfig }: {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-sm font-bold text-gray-400 uppercase tracking-widest">{t('equipment.workShifts')}</h2>
      <p className="text-xs text-gray-600">{t('equipment.shiftsSubtitle')}</p>
      <div className="space-y-3">
        {shifts.map((sh, i) => (
          <div key={sh.key} className="bg-[#0d1421] rounded-2xl border border-white/[0.06] p-4 flex items-center gap-4">
            ...
            <span className="w-24 text-sm font-bold text-white">{t(sh.labelKey)}</span>
            <div className="flex items-center gap-2 flex-1">
              ...
              <span className="text-gray-600 text-xs">{t('common.rangeTo')}</span>
      ...
      <div className="flex items-center gap-3">
        <button onClick={handleSave} disabled={saving} className="btn-primary py-2 px-5 text-sm">
          {saving ? t('common.saving') : t('equipment.shiftsSave')}
        </button>
        {saved && <span className="text-green-400 text-sm">{t('common.saved')}</span>}
      </div>
      <p className="text-xs text-gray-700 italic">{t('equipment.shiftsKioskNote')}</p>
    </div>
  );
}
...
function ParametersTab({ form, set, shiftsConfig }: {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#0d1421] rounded-2xl border border-white/[0.06] p-5 space-y-4">
        <h2 className="text-sm font-bold text-gray-400 uppercase tracking-widest">{t('equipment.paramHourlyRate')}</h2>
        <div className="flex gap-3">
          <div className="flex-1">
            <label className="block text-sm text-gray-500 mb-1.5">{t('equipment.paramRate')}</label>
            <input type="number" min={0} step="0.01" value={form.hourly_rate ?? ''}
              ...
              placeholder={t('equipment.paramRatePlaceholder')}
              className="w-full bg-[#0b1120] border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-blue-500" />
          </div>
          <div>
            <label className="block text-sm text-gray-500 mb-1.5">{t('workOrders.currency')}</label>
      ...
      <div className="bg-[#0d1421] rounded-2xl border border-white/[0.06] p-5 space-y-4">
        <h2 className="text-sm font-bold text-gray-400 uppercase tracking-widest">{t('equipment.paramPanels')}</h2>
        <Toggle label={t('equipment.paramShowProductionPanel')} checked={!!form.show_production_panel} onChange={(v) => set('show_production_panel', v)} />
        <Toggle label={t('equipment.paramShowAvailabilityGauge')} checked={!!form.show_availability_gauge} onChange={(v) => set('show_availability_gauge', v)} />
        <Toggle label={t('equipment.paramShowRejectPanel')} checked={!!form.show_reject_panel} onChange={(v) => set('show_reject_panel', v)} />
        <Toggle label={t('equipment.paramShowJobNumber')} checked={!!form.show_job_number} onChange={(v) => set('show_job_number', v)} />
      </div>
    </div>
  );
}
...
function InterventionTypesConfigTab({ equipmentId }: { equipmentId: string }) {
  ...
  const handleDeleteIT = async (id: string) => {
    if (!confirm(t('equipment.itDeactivateConfirm'))) return;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-bold text-gray-400 uppercase tracking-widest">{t('nav.interventionTypes')}</h2>
        <div className="flex items-center gap-2">
      ...
      {showAdd && (
        <div className="p-4 bg-[#0d1421] rounded-2xl border border-blue-500/30 space-y-3">
          <p className="text-sm font-semibold text-white">{t('equipment.itNewType')}</p>
          <div className="flex flex-wrap gap-3 items-end">
            <div className="flex-1 min-w-[160px]">
              <label className="block text-xs text-gray-500 mb-1">{t('equipment.name')}</label>
              <input value={addForm.name} onChange={(e) => setAddForm({ ...addForm, name: e.target.value })}
                placeholder={t('equipment.itNamePlaceholder')}
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">{t('common.displayOrder')}</label>
          ...
          <div>
            <label className="block text-xs text-gray-500 mb-2">{t('common.icon')}</label>
      ...
      {loadingIT ? (
        ...
        <div className="py-10 text-center text-gray-700 text-sm">{t('equipment.itNone')}</div>
      ) : (
        <div className="space-y-2">
          {types.map((t) =>
            editId === t.id ? (
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-1">
        ...
        <p className="text-sm font-semibold text-gray-200">{t('equipment.safetyChecklist')}</p>
        {checklistId && <span className="text-xs text-gray-600 font-mono">{checklistId.slice(0, 8)}</span>}
      </div>
      <p className="text-xs text-gray-600">
        {t('equipment.safetyChecklistDesc')}
      </p>

      {items.length === 0 && (
        <p className="text-gray-600 text-sm py-2">{t('equipment.safetyNoItems')}</p>
      )}
      ...
      <div className="flex gap-2 items-center">
        <input
          ...
          placeholder={t('equipment.safetyNewItemPlaceholder')}
    ...
    <div className="bg-[#0d1421] border border-white/[0.06] rounded-xl overflow-hidden">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/[0.06] text-xs text-gray-500 uppercase">
              ...
              <th className="px-4 py-3 text-right">{t('equipment.histResponse')}</th>
              <th className="px-4 py-3 text-right">{t('equipment.histDuration')}</th>
              <th className="px-4 py-3 text-right">{t('equipment.histDowntime')}</th>
              <th className="px-4 py-3 text-left">{t('common.notes')}</th>
              <th className="px-4 py-3 text-left">{t('equipment.histTicket')}</th>
            </tr>
          </thead>
          <tbody>
            {items.map(item => (
      ...
      {total > limit && (
        <div className="flex items-center justify-between px-4 py-3 border-t border-white/[0.06] text-xs text-gray-500">
          <span>{t('equipment.histTotal', { total })}</span>
          <div className="flex gap-2">
            <button
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {machine?.page_slug && (
            ...
            >
              <ExternalLink size={12} /> {t('equipment.openKiosk')}
            </Link>
          )}
          {showSave && (
      ...
      <div className="max-w-3xl">
        {configTab === 'general' && (
          <div className="space-y-6">
            <div className="bg-[#0d1421] rounded-2xl border border-white/[0.06] p-5 space-y-4">
              <h2 className="text-sm font-bold text-gray-400 uppercase tracking-widest">{t('equipment.cfgDisplay')}</h2>
              <div>
                <label className="block text-sm text-gray-500 mb-1.5">{t('equipment.cfgDisplayName')}</label>
              ...
              <div>
                <label className="block text-sm text-gray-500 mb-1.5">{t('equipment.cfgPageLanguage')}</label>
                <div className="flex gap-2">
                  {LANG_OPTIONS.map((l) => (
              ...
              <div>
                <label className="block text-sm text-gray-500 mb-1.5">{t('equipment.cfgAccentColor')}</label>
            ...
            <div className="bg-[#0d1421] rounded-2xl border border-red-500/20 p-5 space-y-3">
              <h2 className="text-sm font-bold text-red-400 uppercase tracking-widest">{t('equipment.cfgDangerZone')}</h2>
              <p className="text-xs text-gray-600">{t('equipment.cfgDangerDesc')}</p>
            </div>
          </div>
        )}
        ...
        {configTab === 'indicators' && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-gray-600 text-sm">{t('equipment.cfgIndicatorsSoon')}</p>
          </div>
        )}
...
export default function EquipmentDetail() {
  ...
  const tabs: { id: TabId; label: string }[] = [
    ...
    ...(isAux ? [] : [{ id: 'history' as TabId, label: t('equipment.tabHistory') }]),
    { id: 'configuration',  label: t('equipment.tabConfiguration') },
  ];
            ...
            <div>
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full border ${STATUS_COLORS[equipment.status] ?? STATUS_COLORS.stopped}`}>
                  {STATUS_LABEL[equipment.status]?.[lang] ?? equipment.status.replace('_', ' ')}
                </span>
                {isAux && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="bg-[#0d1421] border border-white/[0.06] rounded-xl p-4 space-y-3">
            ...
            <EditableSpecRow
              ...
              placeholder={t('equipment.functionPlaceholder')}
            ...
            <EditableSpecRow
              ...
              equipmentId={equipment.id}
              field="subtype"
              placeholder={t('equipment.subtypePlaceholder')}
            ...
            <EditableSpecRow
              ...
              type="number"
              placeholder={t('equipment.yearPlaceholder')}
            ...
            <EditableSpecRow
              ...
              field="criticality"
              type="select"
              options={['low', 'medium', 'high', 'critical'].map((c) => ({ value: c, label: t(`priority.${c}`, c.charAt(0).toUpperCase() + c.slice(1)) }))}
            ...
            <EditableSpecRow
              ...
              equipmentId={equipment.id}
              field="family"
              placeholder={t('equipment.familyPlaceholder')}
            ...
            <EditableSpecRow
              ...
              equipmentId={equipment.id}
              field="pm_strategy"
              placeholder={t('equipment.pmStrategyPlaceholder')}
            ...
            <EditableSpecRow
              ...
              options={[
                { value: '', label: '—' },
                { value: 'Basse', label: t('equipment.cleanPriorityLow') },
                { value: 'Moyenne', label: t('equipment.cleanPriorityMedium') },
                { value: 'Haute', label: t('equipment.cleanPriorityHigh') },
              ]}
            ...
            <EditableSpecRow
              ...
              field="height_3d"
              type="number"
              placeholder={t('equipment.height3dPlaceholder')}
...
function ModelUploadRow({ equipment, onSaved }: { equipment: Equipment; onSaved: (url: string | null) => void }) {
  ...
  const onFile = async (f: File) => {
    ...
    try {
      ...
      setErr(msg ?? t('factoryMap.modelUploadFailed'));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="flex items-center justify-between gap-2 py-1.5 border-b border-white/[0.03] last:border-0">
      <span className="text-xs text-gray-600 flex-shrink-0">{t('factoryMap.model3d')}</span>
      <div className="flex items-center gap-2">
        {err ? <span className="text-xs text-red-400">{err}</span>
          : equipment.model_url ? <span className="text-xs text-green-400">✓ {t('equipment.modelUploaded')}</span>
        ...
        <button onClick={() => fileRef.current?.click()} disabled={busy}
          ...
          {busy ? t('equipment.modelUploading') : (equipment.model_url ? t('factoryMap.replaceGlb') : t('factoryMap.uploadGlb3'))}
        </button>
        {equipment.model_url && (
          <button onClick={remove} className="text-xs text-gray-500 hover:text-red-400">{t('common.remove')}</button>
        )}
...
function ParentMachineRow({ equipmentId, currentParentId, onSaved }: {
  ...
  return (
    <div className="flex items-center justify-between gap-2 py-1.5 border-b border-white/[0.03] last:border-0">
      <span className="text-xs text-gray-600 flex-shrink-0">{t('equipment.parentMachine')}</span>
      {editing ? (
        <div className="flex items-center gap-1.5">
          <select autoFocus value={draft} onChange={(e) => setDraft(e.target.value)} disabled={saving} className={`w-44 ${editorCls}`}>
            <option value="">{t('equipment.parentNone')}</option>
