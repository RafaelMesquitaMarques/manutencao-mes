...
- **Replay do turno (2026-09)** — modo adicional do Factory Map que reproduz no mapa um turno/período passado (reproduzir · pausar · acelerar 1×–1800× · saltar para um horário). **Nenhuma tabela nova**: os estados são reconstruídos de `machine_stops` + `machine_interventions` + `maintenance_tickets` com a mesma precedência do live (`live_status.py`), as OFs vêm de `job_order_runs`, a produção de `machine_production_hourly` e o buffer de `pit_stop_movements`. A janela inteira vem num fetch (`/api/factory-replay/{plant}/timeline`) e `overlayAt(t)` alimenta o MESMO `applyStatus()` do push ao vivo, por isso 2D, 3D, cores, legenda e filtros funcionam sem alteração; enquanto o replay corre, WS/polls ao vivo e o modo de edição ficam suspensos. Limitações assumidas (sem log de `current_status`, cobots seguem a máquina-mãe, pilhas 3D do Pit Stop fora do escopo inicial) em `docs/factory-replay.md`. Testes `tests/test_factory_replay.py` + `replay/replayModel.test.ts`.
- **Pit Stop buffer (2026-07)** — the physical staging zone between fabrication and the assembly lines, on the 3D map as an Equipment `block_kind='pit_stop'` (41 instanced roller lanes of 44 ft; geometry/late-threshold/ingest-token in `equipment.specifications`). OFs present in the buffer render as stacks (slab per component, colour = `pit_stop_categories` registry; base plate = OF-state palette in `utils/pitStopColors.ts`, distinct from machine status). Data: `job_order_components` (BOM), `pit_stop_movements` (append-only in/out ledger, anomalies flagged never rejected), `pit_stop_of_states` (priority/hold/release). Read model `GET /api/pit-stop/{plant}/state` (dedicated ~15 s poll, NOT the 4 s WS), ingest `POST /{plant}/ingest` (assumed SAP contract — `docs/pit-stop-sap-contract.md`), actions `PATCH /of/{id}` gated by `pit_stop:update`. Completeness = cumulative received vs BOM (OTIF: in-full + ≥90 % counters). Simulator `scripts/simulate_pit_stop.py` (SIM-PS-*, `--reset`/`--live`); seed `scripts/seed_pit_stop.py`. Tests `tests/test_pit_stop.py`.
...
- **Predictive Intelligence (2026-07)** — layered explainable machine-health engine (`services/predictive/`, `/api/predictive`, page `/predictive` + "Santé prédictive" tab on EquipmentDetail): per-context baselines, trend/anomaly/MTBF/pattern-similarity factors, silent→active activation ladder per plant/machine, technician feedback, backtesting, full audit (snapshots + engine/config versions). 10 additive tables (`predictive_*`, `machine_baselines`, `failure_events`, `failure_patterns`). Design doc: `docs/predictive-intelligence.md`; simulator `scripts/simulate_predictive.py`; tests `tests/test_predictive.py`.
...
│       │   ├── factory_replay.py    # /api/factory-replay/ (replay do turno — só leitura)
...
│       │   │   └── replay/          # Replay do turno (barra de transporte, índice, painéis)
