...
- **Shift replay (2026-09)** — an extra Factory Map mode that plays a past shift/period back on the map (play · pause · speed up 1×–1800× · jump to a time). **No new table**: states are rebuilt from `machine_stops` + `machine_interventions` + `maintenance_tickets` with the same precedence as live (`live_status.py`); OFs come from `job_order_runs`, production from `machine_production_hourly` and the buffer from `pit_stop_movements`. The whole window arrives in one fetch (`/api/factory-replay/{plant}/timeline`) and `overlayAt(t)` feeds the SAME `applyStatus()` as the live push, so 2D, 3D, colors, legend and filters work unchanged; while the replay runs, the live WS/polls and edit mode are suspended. Accepted limitations (no `current_status` log, cobots follow their parent machine, Pit Stop 3D stacks out of the initial scope) in `docs/factory-replay.md`. Tests `tests/test_factory_replay.py` + `replay/replayModel.test.ts`.
...
│       │   ├── factory_replay.py    # /api/factory-replay/ (shift replay — read-only)
...
│       │   │   └── replay/          # Shift replay (transport bar, index, panels)
